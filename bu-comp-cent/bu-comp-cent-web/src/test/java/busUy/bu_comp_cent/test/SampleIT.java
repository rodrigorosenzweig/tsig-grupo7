package busUy.bu_comp_cent.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "bu-comp-cent-ear.ear");

        // Current directory is the root of the "bu-comp-cent-web" project. Go up one level
        // and enter the "bu-comp-cent-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../bu-comp-cent-ejb/target/bu-comp-cent-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "bu-comp-cent-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/bu-comp-cent-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "bu-comp-cent-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("busUy.bu_comp_cent.test");
